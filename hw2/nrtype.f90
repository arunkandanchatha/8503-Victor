MODULE nrtype
    ...
    ABSTRACT INTERFACE
        ...
        FUNCTION template_derivative(point) RESULT(z)
            !
            ! the derivative of the function we are trying to minimize
            ! INPUTS: point - the value at which we are evaluating the function
            ! OUTPUTS: z - the value of the partial derivatives of the function
            !              at that point
            REAL(KIND=8), DIMENSION(:), INTENT(IN) :: point
            REAL(KIND=8), DIMENSION(size(point)) :: z
        END FUNCTION template_derivative
